...
This was an untidy dataset. It consisted of three `.xlsx` files with mismatched columns. Each file had collection of data representing one year. The data was from a survey, which was about candy prerences. This dataset consisted of variables of `charachter` type variables. Only the first two variables: `year` and `age` were represented as numeric types. 

* Variable `year` was missing from file representing year 2017. This data was critical for the analysis I was asked to perform. I have inserted this data manually. 
As the dataset was about "candy", I have removed any variables, which did not resemble candy, e.g. "Cash" or seasons of "Person of Interest" and "Desperate Housewives". 

* Names of variables in different files were mismatched, but represented same data, e.g whether or not someone was going out trick or treating. I consolidated these data under singular variables `trick_or_treating`, `age` and `country`.

* The variable `country` contained very disorganised data. For my analysis I only needed to know if a country was US or CANADA or UK or OTHER. I created three vectors (dictionaries), which I used to classify the countries into required catecories. 
Some information in the variable `countries`was missing. I created a category UNKNOWN for missing data in this variable. 

* Variable `age` contained some entries, wich were clearly inaccurate, i.e. entries over 100 years old and a few entries over 1000. 

I assumed that even though unlikely, but not impossible that someone aged 99 would go trick or treating (possibly with their grandchildren). For this reason I overwritten variable `age` with 99 for any number which was over 99. 

I also renamed `timestamp` to `year`, as I only needed to keep the year, instead of the full date.

* I renamed all variables in consistent format in lowercase with `_` separating the words of the same variable. 
...
most_pop_by_year
```
Again, Any Full Sized Candy Bar was the preferred option over three year period. 

## Answer 8. 
The most popular candy bar in each country, measured on numeric scale.

* I am using the dataframe `numeric_ratings` to answer this question too. 
I have grouped the ratings by country: US, UK, CANADA, OTHER. I added a category "UNKNOWN" for countries which could not be classified as any of the four countries due to missing data.  
```{r}
#find the most popular candy in CANADA
most_pop_canada <- numeric_ratings %>%
  filter(country == "CANADA") %>%
  group_by(candy_name) %>%
  summarise(numeric_rating = sum(numeric_rating))%>%
  arrange(desc(numeric_rating)) %>%
  mutate(country = "CANADA", candy_name, numeric_rating) %>%
  head(1)

#find the most popular candy in US
most_pop_us <- numeric_ratings %>%
  filter(country == "US") %>%
  group_by(candy_name) %>%
  summarise(numeric_rating = sum(numeric_rating))%>%
  arrange(desc(numeric_rating)) %>%
  mutate(country = "US", candy_name, numeric_rating) %>%
  head(1)

#find the most popular candy in UK
most_pop_uk <- numeric_ratings %>%
  filter(country == "UK") %>%
  group_by(candy_name) %>%
  summarise(numeric_rating = sum(numeric_rating))%>%
  arrange(desc(numeric_rating)) %>%
  mutate(country = "UK", candy_name, numeric_rating) %>%
  head(1)

#find the most popular candy in OTHER countries
most_pop_other <- numeric_ratings %>%
  filter(country == "OTHER") %>%
  group_by(candy_name) %>%
  summarise(numeric_rating = sum(numeric_rating))%>%
  arrange(desc(numeric_rating)) %>%
  mutate(country = "OTHER", candy_name, numeric_rating) %>%
  head(1)

#find the most popular candy in UNKNOWN countries. 
most_pop_unknown <- numeric_ratings %>%
  filter(country == "UNKNOWN") %>%
  group_by(candy_name) %>%
  summarise(numeric_rating = sum(numeric_rating))%>%
  arrange(desc(numeric_rating)) %>%
  mutate(country = "UNKNOWN", candy_name, numeric_rating) %>%
  head(1)

#combine all findings into single dataframe for easier readability
most_pop_by_country <- bind_rows(most_pop_canada, 
          most_pop_us, 
          most_pop_uk, 
          most_pop_other,
          most_pop_unknown) %>%
  select(country, candy_name, numeric_rating) %>%
  arrange(numeric_rating)

most_pop_by_country
```
Yet again, the "Any Full Sized Candy Bar" seems to be the overall preferred option in all the country caterories represented in this dataset. 
